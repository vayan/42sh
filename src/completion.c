...
t_completion	*find_commande(char *buf, int pos, int begin, t_hach_bin *hach)
{
  ...
  completion = 0;
  while ((begin - 1) < pos && begin != -1)
    {
      if (hach->next && hach->next[buf[begin - 1] - 32])
	hach = hach->next[buf[begin - 1] - 32];
      else
	begin = -2;
      begin++;
    }
...
t_completion	*find_in_folder(char *buf, int pos, int begin)
{
  ...
  if ((tmp2 == begin || begin == 0) && buf[tmp2] != '/')
    ...
    {
      tmp = xmalloc((pos - tmp2 + 2) * sizeof(*tmp));
...
t_completion	*find_completion(int type, char *buf, int pos, int begin)
{
  ...
  tmp = recup_shell(0);
  if (type)
    return (find_commande(buf, pos, begin, tmp->tab_hach));
  else
